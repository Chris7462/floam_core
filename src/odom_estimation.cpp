...
void OdomEstimation::init(double map_resolution)
{
  ...
  // kd-tree
  kdtree_edge_map_ = pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr(new pcl::KdTreeFLANN<pcl::PointXYZI>());
  kdtree_surf_map_ = pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr(new pcl::KdTreeFLANN<pcl::PointXYZI>());
  ...
  const pcl::PointCloud<pcl::PointXYZI>::Ptr surf_in)
{
  ...
  Eigen::Isometry3d odom_prediction = odom * (last_odom_.inverse() * odom);
  last_odom_ = odom;
  odom = odom_prediction;

  q_w_curr_ = Eigen::Quaterniond(odom.rotation());
  t_w_curr_ = odom.translation();
  ...
  if (lidar_cloud_corner_map->points.size() > 10 &&
      lidar_cloud_surf_map->points.size() > 50) {
    kdtree_edge_map_->setInputCloud(lidar_cloud_corner_map);
    kdtree_surf_map_->setInputCloud(lidar_cloud_surf_map);
    ...
    for (int iter_count = 0; iter_count < optimization_count_; iter_count++) {
      ...
      add_edge_cost_factor(downsampled_edge_cloud, lidar_cloud_corner_map, problem, loss_function);
      add_surf_cost_factor(downsampled_surf_cloud, lidar_cloud_surf_map, problem, loss_function);
  ...
  odom = Eigen::Isometry3d::Identity();
  odom.linear() = q_w_curr_.toRotationMatrix();
  odom.translation() = t_w_curr_;
  ...
  ceres::LossFunction * loss_function)
{
  ...
  for (int i = 0; i < (int)pc_in->points.size(); i++) {
    ...
    kdtree_edge_map_->nearestKSearch(point_temp, 5, point_search_ind, point_search_sq_dis);
  ...
  ceres::LossFunction * loss_function)
{
  ...
  for (int i = 0; i < (int)pc_in->points.size(); i++) {
    ...
    std::vector<int> point_search_ind;
    std::vector<float> point_search_sq_dis;
    kdtree_surf_map_->nearestKSearch(point_temp, 5, point_search_ind, point_search_sq_dis);
  ...
  const pcl::PointCloud<pcl::PointXYZI>::Ptr downsampled_surf_cloud)
{
  ...
  crop_box_filter_.setInputCloud(lidar_cloud_corner_map);
  crop_box_filter_.filter(*tmp_corner);
  crop_box_filter_.setInputCloud(lidar_cloud_surf_map);
  crop_box_filter_.filter(*tmp_surf);

  down_size_filter_edge_.setInputCloud(tmp_corner);
  down_size_filter_edge_.filter(*lidar_cloud_corner_map);
  down_size_filter_surf_.setInputCloud(tmp_surf);
  down_size_filter_surf_.filter(*lidar_cloud_surf_map);
}
